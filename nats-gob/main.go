...
import (
	...
	nats_watermill "github.com/ThreeDotsLabs/watermill-nats/pkg/nats"
...
var usageStr = `
Usage: gob-pub [options] <subject> <message>
Options:
...
// NOTE: Use tls scheme for TLS, e.g. gob-pub -s tls://demo.nats.io:4443 foo hello
func usage() {
	fmt.Printf("%s\n", usageStr)
	os.Exit(0)
}
...
func main() {
	...
	flag.StringVar(&clientID, "id", "gob-pub", "The NATS Streaming client ID to connect with")
	flag.StringVar(&clientID, "clientid", "gob-pub", "The NATS Streaming client ID to connect with")
	...
	subj, origMsg := args[0], []byte(args[1])
	...
	event := message.NewMessage(watermill.NewUUID(), origMsg)

	log.Printf("New message %v", event)

	msg, err := nats_watermill.GobMarshaler{}.Marshal(subj, event)
	if err != nil {
